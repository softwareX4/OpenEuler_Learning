...
当下载后，会有wget-list这个文件，这个文件中列出了构建LFS 10的所有源码包，那么我们就使用一个脚本程序 wget_list.sh,内容如下：
```sh
#!/bin/bash
...
或者选择镜像站并下载软件包，用时34m59s
```sh
wget http://mirrors.ustc.edu.cn/lfs/lfs-packages/10.0/wget-list
wget --input-file=wget-list --continue --directory-prefix=$LFS/sources
wget http://mirrors.ustc.edu.cn/lfs/lfs-packages/MD5SUMS
```
检查软件包
```sh
pushd $LFS/sources
